...
export function NotificationDataTable() {
  ...
  const fetchNotifications = async () => {
    try {
      const response = await fetch('/lib/GET/Notification/getallNotification');
      if (!response.ok) {
  ...
  const fetchPermission = async () => {
    ...
    try {
      ...
      const result = await response.json();
      setIsAuthorized(result.product !== 0);
      setNewData(Array.isArray(result.product) ? result.product : []);
    } catch (error) {
      console.error('Error fetching data:', error);
    }
  };
        ...
        <div>
          ...
          <div className="w-full">
            <div className="flex items-center py-4">
              <Input
                placeholder="Filter by Registration Token..."
                value={(table.getColumn("registrationToken")?.getFilterValue() as string) ?? ""}
                onChange={(event) => table.getColumn("registrationToken")?.setFilterValue(event.target.value)}
                className="max-w-sm"
              />
              <DropdownMenu>
